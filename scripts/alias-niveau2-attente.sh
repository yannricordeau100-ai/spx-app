...
for x in d.get('deployments',[]):
  ...
  case "$R" in
    READY*) U=${R#READY }; npx vercel alias set "https://$U" mettrik-niveau2.vercel.app --token "$TOK" --scope $TEAM 2>&1 | tail -1; echo "NIVEAU 2 A JOUR sur $U ($SHA)"; bash scripts/prechauffe-fiches.sh https://mettrik-niveau2.vercel.app 120 > /tmp/prechauffe.log 2>&1 & exit 0;;
