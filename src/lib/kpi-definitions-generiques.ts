...
const DEFS: Array<[RegExp, string]> = [
  ...
  [/ventes? d.[ée]lectricit[ée]|[ée]lectricit[ée] (livr[ée]e|vendue)|(gwh|kwh|mwh) sales|electric(ity)? (sales|deliver)|retail (electric )?sales|deliveries in franchise area/i, "Quantité d'électricité vendue ou livrée aux clients sur la période, en gigawattheures. C'est le volume de base d'un réseau électrique : le chiffre d'affaires est ce volume multiplié par les tarifs autorisés."],
  [/volumes? (distribu[ée]s?|de gaz)|distribution throughput|gas deliver|\bmmcf\b|\bdth\b|d[ée]catherm/i, "Quantité de gaz naturel acheminée aux clients sur la période (million de pieds cubes ou décatherms). Le volume dépend surtout de la rigueur de l'hiver ; c'est la base d'activité d'un distributeur de gaz."],
  [/volumes? d.eau|water (services )?volumes?|billed water|gallons/i, "Volume d'eau facturé aux clients sur la période, en millions de gallons. Il évolue lentement avec le nombre de clients et la météo ; le chiffre d'affaires en découle par les tarifs autorisés."],
  [/production de cuivre|copper production|(livres|pounds) de cuivre|\bmlb\b/i, "Quantité de cuivre extraite et récupérable sur la période, en millions de livres. Multipliée par le prix du cuivre, elle donne l'essentiel du chiffre d'affaires du mineur."],
  [/croissance (organique|sous.jacente) (du chiffre d.affaires|des commissions)|underlying revenue growth|organic (revenue )?growth/i, "Croissance du chiffre d'affaires à périmètre et taux de change constants, hors acquisitions et cessions. Chez un courtier d'assurance, c'est la mesure de référence de la dynamique commerciale réelle."],
