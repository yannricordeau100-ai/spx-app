#!/usr/bin/env python3
"""SA34 batch 19 (last) — parse 10-Q HTM (inline XBRL) for US ticker BF.B.

For each US ticker in batch19.txt:
...
EU tickers (ADYEN.AS, CCH.L, NN.AS, SOF.BR, ABBN.SW, AD.AS, AKZA.AS, AZN.ST,
FLTR.L, FRE.DE, PAH3.DE):
- Per spec, EU = half-year / annual-text. Need >=4 semester periods.
- Most EU tickers in batch19 have ONLY annual-text (no half-year dir): SKIPPED.
- PAH3.DE has 5 H1 .txt files but unstructured text + KPIs (DPS/Cap Return/Payout
  Ratio) are annual declarations, not semester values. NO reliable extraction.
- All EU tickers SKIPPED for batch19 (consistent with batch07 EU handling).
...
BATCH = '/tmp/sa34-batches/batch19.txt'
TODAY = '2026-06-03'
FIX_LOG_TAG = 'SA34-Claude'

EU_TICKERS = {'ADYEN.AS', 'CCH.L', 'NN.AS', 'SOF.BR', 'ABBN.SW', 'AD.AS',
              'AKZA.AS', 'AZN.ST', 'FLTR.L', 'FRE.DE', 'PAH3.DE'}

# Ticker → SEC filename token (e.g. BF.B → BF-B)
TICKER_FILE_TOKEN = {'BF.B': 'BF-B'}
...
KPI_MAP = {
    ...
    'Net Income': {'concepts': ['NetIncomeLoss'], 'type': 'duration'},
...
def collect_ticker_facts(ticker):
    sec_token = TICKER_FILE_TOKEN.get(ticker, ticker)
    files = []
    for year in ('2023', '2024', '2025', '2026'):
        files.extend(sorted(glob.glob(os.path.join(SEC10Q, year, f'{sec_token}_*.htm.gz'))))
...
def process_us_ticker(ticker, apply_mode=False):
    ...
    for kpi in year_kpis:
        ...
        unit = kpi.get('unit')
        if mode == 'ratio':
            vals = [round(v, 4) for _d, v in series]
        else:
...
def main():
    ...
    print(f'=== SA34 batch19 ({len(tickers)} tickers) apply={apply_mode} ===')
    summary = {'converted': 0, 'skipped': 0, 'unmapped': 0, 'no_year': 0, 'missing': 0, 'eu_skipped': 0}
    for t in tickers:
        print(f'\n[{t}]')
        if t in EU_TICKERS:
            print('  EU ticker: no half-year XBRL source available (annual-text only or unstructured H1 .txt). SKIPPED.')
            summary['eu_skipped'] += 1
            continue
        lines = process_us_ticker(t, apply_mode=apply_mode)
        for ln in lines:
            print(ln)
            if 'CONVERTED' in ln:
                summary['converted'] += 1
