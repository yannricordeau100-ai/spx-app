...
0) KPI phares par industrie (alias « industrie ») fournis par le propriétaire via une autre IA : les INTÉGRER d abord, sans aucune recherche de données (source à indiquer par Yann au lancement ; si le fichier n est pas fourni, demander et passer au point 1).
0bis) KPI des 74 industries GICS (référentiel docs/cahier/kpi-referentiel-gics-74.json, état affiché dans l onglet « KPI par industrie » de /sandbox/gics) : pour CHAQUE industrie, d abord vérifier si le KPI existe déjà dans les fiches de cette industrie (l état pré-calculé est dans src/data/kpi-industries-etat.json) ; si oui, ne pas l ajouter et le marquer comme déjà présent dans l onglet ; si non, chercher la donnée pour CHAQUE société de l industrie, sur 5 ans, en trimestriel si publié, sinon semestriel, sinon annuel. Les KPI de niveau sous-industrie déjà écrits (docs/cahier/kpi/) sont un travail DIFFÉRENT, trop fin : ne pas les mélanger. Chaque KPI complété pour une société passe en vert dans l onglet (légende en haut). Tenir à jour le compte par industrie, par secteur et le total.
...
5) Types de KPI, alignés sur le référentiel officiel docs/cahier/kpi-referentiel-gics-74.json (74 industries GICS, 1 044 KPI FR/EN fournis par le propriétaire) : chaque type retenu porte le libellé du référentiel quand il existe, sinon un libellé nouveau signalé dans le rapport (définitions validées sur /sandbox/kpi-definitions ; travail parfait exigé sur TOUS les KPI avancés) : classer chaque KPI IC (standard + avancés) en type comparable ou KPI unique avec la règle du concurrent (un concurrent peut le publier, même en théorie, suffit) ; généraliser les KPI uniques dont l actif sous-jacent est comparable (ex : part de marché de Chrome -> part de marché des navigateurs web) ; poser le type dans un champ invisible, l afficher dans le « i » et le brancher dans le Comparer (remplace la clé par libellé du point 1). Compter KPI total, KPI IC total et Types de KPI (sur l univers et par société). Ex validé : villes desservies par Waymo -> villes couvertes par un service de robotaxi. Travail minutieux, Opus puis Sonnet vérificateur, échantillon relu.
