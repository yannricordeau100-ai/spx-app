#!/usr/bin/env python3
"""
Retry 10 failed batch 15 translations via Cerebras Qwen-3-235B (free).
Used because Anthropic credit balance is depleted (§0bis: zero paid Anthropic).
"""
...
def main():
    ...
    print(f"[retry-cerebras] {len(FAILED)} failed | keys={len(CEREBRAS_KEYS)}", flush=True)
    t0 = time.time()
    done = 0
    failed = 0
    failed_list = []
    ...
    for ticker, locale in FAILED:
        ...
        print(f"[try] {ticker}.{locale}", flush=True)
        ...
        translated["_translated_by"] = "cerebras-qwen3-batch15-retry"
