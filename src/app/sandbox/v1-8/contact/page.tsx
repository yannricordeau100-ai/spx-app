...
/**
 * /sandbox/v1-8/contact — page contact V1.8.
 *
 * Caractéristiques (Yann 8 mai 2026) :
 *  - Auth requise (redirect vers /?auth=signin si pas connecté)
 *  - Acceptation CG obligatoire (checkbox)
 *  - Réutilise le formulaire de /contact (recipient + name + subject + body)
 *  - Stockage dans Supabase `desk_contact_messages` (déjà existant)
 *  - Email notification à l'équipe via Resend
 *  - Multi-langue FR / EN / DE (DE à compléter au seed des traductions)
 *
 * À itérer ensuite (phase K) : système d'édition contenu page via desk.
 */
export default async function SandboxV18ContactPage() {
  const supabase = await createSupabaseServerClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) {
    redirect("/?auth=signin&next=/sandbox/v1-8/contact");
  }
