#!/usr/bin/env python3
"""SA33 batch 15 US — parse 10-Q HTM (inline XBRL) for quarterly data.

For each ticker in batch15.txt:
- Open ~/spx-app/src/data/v2-pipeline/<lowercase>.json
- Find KPIs with period_type='year' that map to known XBRL concepts
- Parse all 10-Q .htm.gz files in ~/Mettrik/sec-data/cat1-us/10Q/{2023,2024,2025,2026}/<TICKER>_*.htm.gz
...
BATCH = '/tmp/sa33-batches/batch15.txt'
TODAY = '2026-06-03'
FIX_LOG_TAG = 'SA33-Claude'

# Map normalized KPI short -> list of XBRL concept candidates (us-gaap local names),
# and aggregation type: 'instant' (balance sheet point-in-time) or 'duration' (single-Q flow)
KPI_MAP = {
...
def parse_facts(html, contexts, concept_locals):
    ...
    for m in NUMRE.finditer(html):
        ...
        if ctx.get('has_segment'):
            # skip segmented facts (only entity totals)
            continue
        try:
...
def quarter_label(end_date):
    """Convert YYYY-MM-DD end date to FY/Q label using calendar quarters."""
    y, m, d = end_date.split('-')
    m = int(m)
    q = (m - 1) // 3 + 1
    return f'{y}Q{q}', end_date


def is_quarter_duration(start, end):
    """Check if start..end spans roughly one quarter (80-100 days)."""
    try:
...
def collect_ticker_facts(ticker):
    """For ticker, scan all 10-Q files, return:
       facts[concept_local] = dict of {period_key -> value} dedup-ed by period.
       For 'instant' concepts, period_key = end date.
       For 'duration' concepts, period_key = end date (only single-Q durations kept).
    """
    files = []
    for year in ('2023', '2024', '2025', '2026'):
        files.extend(sorted(glob.glob(os.path.join(SEC10Q, year, f'{ticker}_*.htm.gz'))))
    facts_instant = defaultdict(dict)   # concept -> {end_date: value}
    facts_duration = defaultdict(dict)  # concept -> {end_date: value}
    for fp in files:
        try:
        ...
        for local, val, ctx, decimals, unitref, scale in parse_facts(html, contexts, concepts_needed):
            if 'instant' in ctx:
                # keep latest per filing for that instant
                d = ctx['instant']
...
def build_quarterly_series(facts_by_date, max_count=20):
    """Take {date: value}, return list newest->oldest of values trimmed to max_count, with dates."""
    items = sorted(facts_by_date.items(), key=lambda x: x[0], reverse=True)
    return items[:max_count]


def scale_value(short, val, unit):
    """Scale raw USD value to KPI's pipeline unit.
    Unit hints: 'Mds $'/'Mds €' -> divide by 1e9; 'M $'/'M €' -> divide by 1e6.
    DPS-like -> raw.
    """
    u = (unit or '').lower()
    if short == 'DPS':
    ...
    if 'mds' in u or 'bn' in u or 'b$' in u or 'b €' in u:
        return val / 1e9
    if u.startswith('m ') or u == 'm$' or u == 'm€' or u.startswith('m ') or 'million' in u:
        return val / 1e6
...
def detect_unit(kpi):
    """Return canonical unit hint for sanity-check."""
    u = (kpi.get('unit') or '').lower()
    return u
...
def process_ticker(ticker, apply_mode=False):
    ...
    for kpi in year_kpis:
        ...
        if short not in KPI_MAP:
            ...
            continue
        kmap = KPI_MAP[short]
        # Try each concept candidate, pick first with >=4 trims
        picked = None
        for c in kmap['concepts']:
        ...
        # Validation: if KPI has current value, compare top of series to it as sanity
        kpi['period_type'] = 'quarter'
        kpi['history'] = vals
        # last_data_date update
...
def main():
    ...
    print(f'=== SA33 batch15 US ({len(tickers)} tickers) apply={apply_mode} ===')
    summary = {'converted': 0, 'skipped': 0, 'unmapped': 0}
    for t in tickers:
        print(f'\n[{t}]')
