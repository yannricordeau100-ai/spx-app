...
function LogoTilt({ ticker }: { ticker: string }) {
  ...
  return (
    <div
      ...
      className="size-16 shrink-0 rounded-2xl border border-[#1f1f1f] bg-[#0a0a0a] p-2 transition-shadow duration-300 hover:shadow-[0_8px_32px_rgba(0,0,0,0.6)] sm:size-20"
      style={{
...
export function CompanyHeader({ company }: { company: Company }) {
  ...
  return (
    <div className="mb-8">
      ...
      {/* Stat chips on a calm row, no hover noise */}
      <div className="mt-5 flex flex-wrap items-center gap-2">
        <StatChip label="Rang mondial" value={company.ranks.global_world} />
        <StatChip label="Rang USA" value={company.ranks.global_us} />
        <StatChip label={`Secteur (${company.sector})`} value={company.ranks.sector} />
        <StatChip label={`Sous-secteur (${company.subsector})`} value={company.ranks.subsector} />
        <StatChip label="Fondée" value={String(company.founded)} />
        <StatChip label="IPO" value={String(company.ipo)} />
      </div>
    </div>
  );
}
