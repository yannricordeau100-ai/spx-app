#!/usr/bin/env python3
"""
enrich-geo-v18-pipeline.py — extrait revenue_by_geography au format strict.
...
Usage : python3 scripts/enrich-geo-v18-pipeline.py [--limit N]
"""
import argparse
import gzip
import json
...
PENDING = Path("/tmp/geo-pending.txt")
LOG = PROJECT_ROOT / ".conv-state/CONV-DATA-geo.log"
...
def extract_geo_section(text: str, max_chars: int = 12000) -> str:
    if not text:
        return ""
    patterns = [
        # Section explicite "geographic"
        r"geographic\s+information",
        r"geographic\s+data",
        r"net\s+sales\s+by\s+geographic",
        r"revenue\s+by\s+geographic",
        r"revenues?\s+by\s+region",
        r"sales\s+by\s+region",
        r"geographic\s+segment\s+(?:revenue|information|sales)",
        r"net\s+sales\s+by\s+reportable\s+segment",  # AAPL style (segments = régions)
        ...
        r"umsatz\s+nach\s+region",
        r"umsatzerl[öo]se\s+nach\s+regionen",
        r"vendite\s+per\s+area",
        r"ricavi\s+per\s+area",
        r"ingresos\s+por\s+regi[oó]n",
    ]
    matches = []
    for pat in patterns:
    ...
    if not matches:
        # Fallback: cherche la proximité "Americas" + "Europe" (signature géo)
        am = list(re.finditer(r"\bAmericas?\b", text, re.IGNORECASE))
        eu = list(re.finditer(r"\bEurope\b", text, re.IGNORECASE))
        for a in am:
            for e in eu:
                if 0 < abs(a.start() - e.start()) < 5000:
                    start = min(a.start(), e.start()) - 500
                    return re.sub(r"\s+", " ", text[max(0, start):max(0, start) + max_chars + 2000])[:max_chars]
        return ""
    matches.sort()
    # Choisir le meilleur contexte : score chaque match par densité de chiffres
    # dans les 4000 chars suivants. Le glossaire/index = pauvre en chiffres.
    # MD&A / Notes financières = riche en chiffres. Évite de matcher dans
    # le glossaire qui ne contient pas la data réelle.
    def score(pos):
...
GEO_PROMPT = """Tu extrais la RÉPARTITION GÉOGRAPHIQUE du chiffre d'affaires de {ticker} depuis ces extraits.

Retourne UNIQUEMENT un JSON strict :
{{
  "unit": "B$" | "M$" | "M€" | "Mds €" | "B€" | etc.,
  "source_date": "YYYY-MM-DD" (date de clôture exercice),
  "source": "10-K Item 7 — Geographic Information" ou équivalent,
  "slices": [
    {{"label": "Americas", "value": 167.0}},
    {{"label": "Europe", "value": 96.7}},
    {{"label": "Greater China", "value": 66.9}},
    {{"label": "Japan", "value": 25.1}},
    {{"label": "Rest of Asia Pacific", "value": 28.3}}
  ]
}}

Règles STRICTES :
- value en CHIFFRES NUMÉRIQUES uniquement (pas null, pas "n/a", pas string)
- Si une région n'a pas de valeur chiffrée → NE PAS l'inclure dans slices
- slices minimum 2 régions avec values valides, sinon retourner {{}}
- unit unique pour toutes les slices (convertir si nécessaire)
...
def main():
    ...
    log_line(f"START : {len(pending)} stés geo (sleep {SLEEP_BETWEEN_CALLS}s, 1 proc)")
