#!/usr/bin/env python3
"""SA34 batch 07 — parse 10-Q HTM (inline XBRL) for US tickers, EU annual-text for EU.

US tickers: convert period_type='year' KPIs to 'quarter' from 10-Q .htm.gz when
≥4 quarterly periods can be extracted.
EU tickers (ABVX, ARGX, BUD, CHKP): would need ≥4 half-year periods which are
unavailable (only annual-text dirs with 1 annual file). SKIPPED.

NEVER invent. _fix_log tag = "SA34-Claude 2026-06-03".
...
BATCH = '/tmp/sa34-batches/batch07.txt'
TODAY = '2026-06-03'
FIX_LOG_TAG = 'SA34-Claude'

EU_TICKERS = {'ABVX', 'ARGX', 'BUD', 'CHKP'}
...
def process_ticker(ticker, apply_mode=False):
    log_lines = []
    # EU skip — need half-year, unavailable
    if ticker in EU_TICKERS:
        return ['SKIP EU (no half-year source, only 1 annual-text file)']
    try:
...
def main():
    ...
    print(f'=== SA34 batch07 ({len(tickers)} tickers) apply={apply_mode} ===')
