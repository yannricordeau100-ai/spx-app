#!/usr/bin/env python3
"""SA36 batch 06 — parse 10-Q HTM (inline XBRL) for US tickers, EU annual/semi for EU.

US tickers: convert period_type='year' KPIs to 'quarter' from 10-Q .htm.gz when
≥4 quarterly periods can be extracted.
EU tickers (CPR.MI): need ≥4 half-year or quarterly periods; if only annual-text
available, SKIP.
...
BATCH = '/tmp/sa36-batches/batch06.txt'
TODAY = '2026-06-03'
...
def process_ticker(ticker, apply_mode=False):
    ...
    if ticker in EU_TICKERS:
        ...
        # Check for half-year, quarterly, interim, ad-hoc
    ...
    for kpi in year_kpis:
        ...
        if short not in KPI_MAP:
            ...
            continue
        kmap = KPI_MAP[short]
        unit_chk = (kpi.get('unit') or '').strip()
        # Skip if KPI is a ratio/percentage but we'd extract absolute flow/stock values
        if unit_chk in ('%', 'x', 'ratio') or unit_chk.endswith(' %'):
            log_lines.append(f'  - {short}: SKIP (unit {unit_chk!r} incompatible with absolute XBRL fact)')
            continue
        picked = None
        for c in kmap['concepts']:
...
def main():
    ...
    print(f'=== SA36 batch06 ({len(tickers)} tickers) apply={apply_mode} ===')
