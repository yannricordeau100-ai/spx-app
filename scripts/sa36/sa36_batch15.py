#!/usr/bin/env python3
"""SA36 batch 15 — parse 10-Q HTM (inline XBRL) for US tickers, EU annual/semi for EU.

US tickers: convert period_type='year' KPIs to 'quarter' from 10-Q .htm.gz when
≥4 quarterly periods can be extracted.
EU tickers (FLTR.L, FRE.DE): need ≥4 half-year or quarterly periods; if only annual-text
available, SKIP.
...
BATCH = '/tmp/sa36-batches/batch15.txt'
TODAY = '2026-06-03'
FIX_LOG_TAG = 'SA36-Claude'

EU_TICKERS = {'FLTR.L', 'FRE.DE'}
...
KPI_MAP = {
    ...
    'Total revenues': {'concepts': ['Revenues', 'RevenueFromContractWithCustomerExcludingAssessedTax', 'SalesRevenueNet'], 'type': 'duration'},
    'Cost of Sales': {'concepts': ['CostOfGoodsAndServicesSold', 'CostOfRevenue'], 'type': 'duration'},
    'Operating Income': {'concepts': ['OperatingIncomeLoss'], 'type': 'duration'},
    'Op Income': {'concepts': ['OperatingIncomeLoss'], 'type': 'duration'},
...
def process_ticker(ticker, apply_mode=False):
    log_lines = []
    # EU skip if only annual-text (need ≥4 half-year/quarterly)
...
def main():
    ...
    print(f'=== SA36 batch15 ({len(tickers)} tickers) apply={apply_mode} ===')
    summary = {'converted': 0, 'skipped_no_data': 0, 'unmapped': 0, 'eu_skipped': 0, 'no_year': 0, 'missing': 0}
    for t in tickers:
        print(f'\n[{t}]')
