#!/usr/bin/env python3
"""SA36 batch 09 — parse 10-Q HTM (inline XBRL) for US tickers, EU annual/semi for EU.

US tickers: convert period_type='year' KPIs to 'quarter' from 10-Q .htm.gz when
>=4 quarterly periods can be extracted.
EU tickers in batch09 (LAND.L, LOGN.SW, NDA-FI.HE, PHIA.AS): only have
annual-report/annual-text dirs. No half-year, no interim, no quarterly. SKIP.
...
BATCH = '/tmp/sa36-batches/batch09.txt'
TODAY = '2026-06-03'
FIX_LOG_TAG = 'SA36-Claude'

# EU tickers in batch09 — handled separately, will SKIP if no half-year/quarterly/interim/ad-hoc
EU_TICKERS = {'LAND.L', 'LOGN.SW', 'NDA-FI.HE', 'PHIA.AS'}

# Map KPI short -> {concepts, type, [denom]}.
# type='instant' (balance sheet point-in-time) | 'duration' (single-Q flow) | 'ratio_pct'
KPI_MAP = {
    ...
    'Cash and Marketable Securities': {'concepts': ['CashCashEquivalentsRestrictedCashAndRestrictedCashEquivalents', 'CashAndCashEquivalentsAtCarryingValue'], 'type': 'instant'},
    ...
    'Interest Income': {'concepts': ['InterestIncomeOperating', 'InvestmentIncomeInterest'], 'type': 'duration'},
    # Ratios (numerator/denominator * 100), both duration, same end_date
    ...
    'R&D % Sales': {'concepts': ['ResearchAndDevelopmentExpense'], 'denom': ['Revenues', 'RevenueFromContractWithCustomerExcludingAssessedTax', 'SalesRevenueNet'], 'type': 'ratio_pct'},
    ...
    'Dividend Per Share': {'concepts': ['CommonStockDividendsPerShareDeclared'], 'type': 'instant'},
}
...
def scale_value(short, val, unit):
    u = (unit or '').lower()
    if 'DPS' in short or 'Dividend Per Share' in short:
        return val
...
def process_ticker(ticker, apply_mode=False):
    log_lines = []
    # EU: skip when only annual sources (need >=4 half-year/quarterly/interim/ad-hoc periods)
    if ticker in EU_TICKERS:
        eu_dir = os.path.join(EU_BASE, ticker)
        if not os.path.isdir(eu_dir):
            return ['SKIP EU (no source dir)']
        sub_dirs_with_periods = []
        for sub in ('half-year', 'quarterly', 'interim', 'ad-hoc'):
            d = os.path.join(eu_dir, sub)
            if os.path.isdir(d):
                files = os.listdir(d)
                if files:
                    sub_dirs_with_periods.append((sub, len(files)))
        if not sub_dirs_with_periods:
            return ['SKIP EU (only annual-text/annual-report, no half-year/quarterly source for >=4 periods)']
        return [f'EU sources present but extractor not implemented: {sub_dirs_with_periods}']
    try:
    ...
    for kpi in year_kpis:
        ...
        unit = kpi.get('unit')
        # series is newest-first; reverse to oldest-first per convention
...
def main():
    ...
    print(f'=== SA36 batch09 ({len(tickers)} tickers) apply={apply_mode} ===')
