#!/usr/bin/env python3
"""Cloud chunk 18 — same logic as chantier-top10-work/extract.py, different TICKERS/LOG."""
import json, os, re, gzip, sys
from datetime import datetime

TICKERS = ["NVR","NWS","NWSA","NXPI","ODFL","OKE","OMC","ON","ORCL","ORLY","OTIS","OXY","PANW","PAYX","PCAR"]
XBRL_TICKER = {}

ENRICH_DIR = "/Users/yann/spx-app/src/data/v2-pipeline-enrich"
DATALAKE = "/Users/yann/spx-app/data-lake"
LOG = "/Users/yann/spx-app/.conv-state/chantier-quarters-log-cloud18.json"
...
def scale_to_M(value, unit_target):
    if unit_target == "$M": return round(value / 1e6, 2)
    elif unit_target == "$": return round(value, 4)
    elif unit_target == "%": return round(value * 100, 2)
    elif unit_target == "employees": return int(value)
    return value
...
HEAD_RE = re.compile(
    ...
    re.IGNORECASE)
HEAD_RE2 = re.compile(r"([\d,]{3,10})\s+(?:full[-\s]time\s+)?(?:regular\s+)?employees\s+(?:worldwide|globally|in\s+total)", re.IGNORECASE)
HEAD_RE3 = re.compile(r"(?:As of\s+[A-Z][a-z]+\s+\d+,?\s+\d{4}[^.]{0,180}?)(?:had|employed|we\s+had|we\s+employed)\s+(?:approximately\s+)?([\d,]{3,10})", re.IGNORECASE)
HEAD_RE4 = re.compile(r"(?:employee\s+headcount|total\s+headcount|our\s+headcount|full[-\s]time\s+employees|headcount)\s+[^.]{0,80}?(?:was|were|of|:|totaled|is|reached)\s+(?:approximately\s+)?([\d,]{3,10})", re.IGNORECASE)
HEAD_RE_HC = re.compile(r"headcount\s+was\s+(?:approximately\s+)?([\d,]{3,10})", re.IGNORECASE)
HEAD_RE_TSLA = re.compile(r"employees\s+worldwide\s+was\s+([\d,]{3,10})", re.IGNORECASE)
HEAD_RE5 = re.compile(r"As of [A-Z][a-z]+ \d+,?\s+\d{4}[^.]{0,300}?we\s+had\s+(?:approximately\s+)?([\d,]{3,10})\s+(?:full[- ]time\s+)?(?:equivalent\s+)?employees", re.IGNORECASE)
...
def extract_headcount_from_10k(ticker):
        ...
        def scan(section, offset=0):
            ...
            for rx in (HEAD_RE_HC, HEAD_RE_TSLA, HEAD_RE5, HEAD_RE3, HEAD_RE4, HEAD_RE, HEAD_RE2):
                for mm in rx.finditer(section):
                    ...
                    if LAYOFF_NEAR.search(ctx_before): continue
                    try:
    ...
    return ([s[1] for s in series], [f"FY{s[0]}" for s in series], series[-1][2])
