...
export async function loadV17Company(
  ...
  if (enrich) {
    ...
    // Yann 28 mai 2026 — MERGE *_fr écrits par 17 sub-agents successifs.
    // Les sub-agents ont écrit dans enrich.json les champs FR suivants qui
    // n'étaient PAS câblés côté SSR (donc invisibles côté UI). On les merge
    // ici. Locale-aware quand pertinent : on bascule la donnée FR sur les
    // champs in-place lus par les composants quand locale = fr (ou fr-fr),
    // et on alimente toujours tagline_i18n.fr (consommé conditionnellement).
    const reqLocale = (opts.locale ?? "").toLowerCase();
    const isFr = reqLocale === "fr" || reqLocale === "fr-fr" || reqLocale === "";

    // 1. tagline_fr → tagline_i18n.fr (composant CompanyHeader lit tagline_i18n.fr).
    if (typeof enrich.tagline_fr === "string" && enrich.tagline_fr.trim()) {
      const cur = ((data as Record<string, unknown>).tagline_i18n as
    ...
    if (
      ...
      enrich.hero_kpi_rationale_fr.trim()
    ) {
      (data as Record<string, unknown>).hero_kpi_rationale =
        enrich.hero_kpi_rationale_fr;
    }

    // 3. events_fr → match par date avec data.events, set title/description in-place
    // quand locale=fr. Format enrich.events_fr : array d'objets
    // [{date, title_fr, description_fr}, ...]
    if (isFr && Array.isArray(enrich.events_fr) && Array.isArray(data.events)) {
      const evFr = enrich.events_fr as Array<{
        date?: string;
        title_fr?: string;
        description_fr?: string;
      }>;
      const byDate = new Map<string, { title_fr?: string; description_fr?: string }>();
      for (const e of evFr) {
        if (e && typeof e === "object" && typeof e.date === "string") {
          byDate.set(e.date, { title_fr: e.title_fr, description_fr: e.description_fr });
        }
      }
      (data as Record<string, unknown>).events = (
        ...
        return {
          ...ev,
          title: match.title_fr && match.title_fr.trim() ? match.title_fr : ev.title,
          description:
            match.description_fr && match.description_fr.trim()
    ...
    // 4. risks_rationale_fr → dict {index: rationale_fr}. Set score_rationale
    // in-place sur data.risks[i] quand locale=fr.
    if (
      isFr &&
    ...
    // 5. kpis_explanation_fr / kpis_description_fr / kpis_signal_fr : dicts
    // {short: value_fr}. Set explanation/description/signal in-place sur les KPIs
    // matching par .short, locale=fr.
    if (isFr && Array.isArray(data.kpis)) {
      const expDict =
    ...
    // 6. ai_positioning_fr.{summary_fr, evidence_fr} → ai_positioning.summary/evidence
    // in-place quand locale=fr.
    if (
      isFr &&
      ...
      const aiFr = enrich.ai_positioning_fr as {
        summary_fr?: unknown;
        evidence_fr?: unknown;
      };
      const ai = (data as Record<string, unknown>).ai_positioning as
        ...
        | undefined;
      if (ai) {
        if (typeof aiFr.summary_fr === "string" && aiFr.summary_fr.trim().length > 0) {
          ai.summary = aiFr.summary_fr;
        }
        if (Array.isArray(aiFr.evidence_fr) && aiFr.evidence_fr.length > 0) {
          ai.evidence = aiFr.evidence_fr;
        }
      }
    }

    // 7. stories_body_fr → array [{kpi_short, body_fr}]. Match par kpi.short avec
    // data.kpis (les stories KPIs sont déjà mergés dans data.kpis avec is_short_history=true).
    // Set story.body in-place quand locale=fr.
    if (isFr && Array.isArray(enrich.stories_body_fr) && Array.isArray(data.kpis)) {
      const storiesArr = enrich.stories_body_fr as Array<{
        kpi_short?: string;
        body_fr?: string;
      }>;
      ...
      for (const s of storiesArr) {
        if (
          s &&
          typeof s === "object" &&
          typeof s.kpi_short === "string" &&
          typeof s.body_fr === "string" &&
          s.body_fr.trim().length > 0
        ) {
          byShort.set(s.kpi_short, s.body_fr);
        }
      }
      if (byShort.size > 0) {
        data.kpis = (data.kpis as AnyKPI[]).map((k) => {
    ...
    // 8. CRITIQUE — kpis_supplementary (array nouveaux KPIs spec) : APPEND à
    // data.kpis avec anti-doublon sur le champ `short`. Source unique des KPIs
    // spécifiques pour ~500 stés (sub-agents batch b1..bN du 28 mai 2026).
    if (Array.isArray(enrich.kpis_supplementary) && Array.isArray(data.kpis)) {
      const existingShortsSupp = new Set(
        (data.kpis as AnyKPI[]).map((k) => k?.short).filter(Boolean),
      );
      const extraSupp = (enrich.kpis_supplementary as AnyKPI[])
        .filter(
          (k) =>
            k &&
            typeof k === "object" &&
            ...
            k.short &&
            !existingShortsSupp.has(k.short),
        )
        .map((k) => ({
          ...k,
          history: normalizeHistory(k.history),
