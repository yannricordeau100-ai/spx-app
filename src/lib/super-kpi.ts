...
function ruleOf40(c: Company): SuperKpi {
  ...
  return {
    ...
    category: "Croissance",
    value: score,
    display: `${score.toFixed(1).replace(".", ",")}`,
    tier,
    color: TIER_COLOR[tier],
    tierLabel: TIER_LABEL[tier],
    gaugePct: gauge,
    inputs: [`Revenue YoY ${revYoY.toFixed(1).replace(".", ",")} %`, `Marge ${marginV.toFixed(1).replace(".", ",")} %`],
...
function qualityOfCompounding(c: Company): SuperKpi {
  ...
  return {
    ...
    category: "Composite",
    value: score,
    display: `${score.toFixed(2).replace(".", ",")}`,
    tier,
    color: TIER_COLOR[tier],
    tierLabel: TIER_LABEL[tier],
    gaugePct: gauge,
    inputs: [`CAGR 5y ${cagrPct.toFixed(1).replace(".", ",")} %`, `Marge ${marginV.toFixed(1).replace(".", ",")} %`],
...
function concentrationRisk(c: Company): SuperKpi {
  ...
  return {
    ...
    tierLabel: TIER_LABEL[tier],
    gaugePct: gauge,
    inputs: [`Top segment : ${top.name}`, `Revenue total : ${revV.toFixed(1).replace(".", ",")}`],
    formula: "Plus gros segment / Revenue total (%)",
    benchmark: "< 35 % = bien diversifié · 35-60 % = concentré · > 60 % = monoculture",
    interpretation:
      pct < 35
        ...
        : pct < 60
        ? `Concentration significative sur ${top.name} (${pct.toFixed(0).replace(".", ",")} %). À surveiller : un choc sectoriel sur ce segment impacterait la totalité.`
        : `Monoculture sur ${top.name} (${pct.toFixed(0).replace(".", ",")} %). La sté est exposée au cycle de ce segment unique. Risque structurel binaire.`,
  };
}
...
function capitalIntensity(c: Company): SuperKpi {
  ...
  return {
    ...
    tierLabel: label,
    gaugePct: gauge,
    inputs: [`Capex ${capexV.toFixed(1).replace(".", ",")}`, `Revenue ${revV.toFixed(1).replace(".", ",")}`],
    formula: "Capex / Revenue (%)",
...
function profitPowerIndex(c: Company): SuperKpi {
  ...
  return {
    ...
    category: "Composite",
    value: score100,
    display: `${score100.toFixed(0).replace(".", ",")} / 100`,
    tier,
    color: TIER_COLOR[tier],
    tierLabel: TIER_LABEL[tier],
    gaugePct: score100,
    inputs: [
      `Rule of 40 : ${r40.value.toFixed(1).replace(".", ",")}`,
      `Marge : ${marginV.toFixed(1).replace(".", ",")} %`,
      `Concentration : ${conc.value.toFixed(1).replace(".", ",")} %`,
      `Tendance marge : ${marginTrend >= 0 ? "+" : ""}${marginTrend.toFixed(1).replace(".", ",")} bps YoY`,
    ],
...
function tacRatio(c: Company): SuperKpi {
  ...
  return {
    ...
    inputs: [`TAC ${t.toFixed(1).replace(".", ",")}`, `Revenue ${r.toFixed(1).replace(".", ",")}`],
...
function cloudPerCapex(c: Company): SuperKpi {
  ...
  return {
    ...
    category: "Stratégie",
    value: ratio,
    display: `${ratio.toFixed(2).replace(".", ",")} ×`,
    tier,
    color: TIER_COLOR[tier],
    tierLabel: ratio >= 1 ? "Harvest" : ratio >= 0.6 ? "Transition" : "Pari massif",
    gaugePct: Math.min(100, ratio * 50),
    inputs: [`Cloud ${cl.toFixed(1).replace(".", ",")} Mds`, `Capex ${cx.toFixed(1).replace(".", ",")} Mds`],
...
function adEngineSaturation(c: Company): SuperKpi {
  ...
  return {
    ...
    category: "Stratégie",
    value: pct,
    display: `${pct.toFixed(0).replace(".", ",")} %`,
    tier,
    color: TIER_COLOR[tier],
    tierLabel: pct < 90 ? "Upsell room" : pct < 110 ? "Mature" : "Saturé",
    gaugePct: Math.min(100, (pct / 130) * 100),
    inputs: [`ARPP ${a}`, `DAP ${d} Mds`, `Revenue ${r.toFixed(1).replace(".", ",")} Mds`],
...
function realityLabsBurn(c: Company): SuperKpi {
  ...
  return {
    ...
    inputs: [`Pertes RL ${lossV.toFixed(1).replace(".", ",")} Mds`, `Revenue ${r.toFixed(1).replace(".", ",")} Mds`],
...
function subscriptionQuality(c: Company): SuperKpi {
  ...
  return {
    ...
    inputs: [`Part subscription ${((sR / tR) * 100).toFixed(0).replace(".", ",")} %`, `Rétention ${re} %`],
...
function spgiMixPremium(c: Company): SuperKpi {
  ...
  return {
    ...
    inputs: [`MI + Indices + Mobility ${(m + i + mb).toFixed(0).replace(".", ",")} M$`, `Revenue ${r.toFixed(0).replace(".", ",")} M$`],
...
function vitalityIndex(c: Company): SuperKpi {
  ...
  return {
    ...
    tierLabel: TIER_LABEL[tier],
    gaugePct: Math.min(100, (pct / 18) * 100),
    inputs: [`Vitality ${v.toFixed(0).replace(".", ",")} M$`, `Revenue ${r.toFixed(0).replace(".", ",")} M$`],
...
function backlogCoverage(c: Company): SuperKpi {
  ...
  return {
    ...
    category: "Croissance",
    value: months,
    display: `${months.toFixed(1).replace(".", ",")} mois`,
    tier,
    color: TIER_COLOR[tier],
    tierLabel: TIER_LABEL[tier],
    gaugePct: Math.min(100, (months / 18) * 100),
    inputs: [`Backlog ${b.toFixed(1).replace(".", ",")} Mds`, `Revenue ${r.toFixed(1).replace(".", ",")} Mds`],
...
function cashQuality(c: Company): SuperKpi {
  ...
  return {
    ...
    inputs: [`FCF ${f.toFixed(1).replace(".", ",")} Mds`, `Net Income ${n.toFixed(1).replace(".", ",")} Mds`],
