#!/usr/bin/env python3
"""Extension du fill governance heuristique au scope étendu :
- top307 (default, comportement #53)
- sp500_hors_top307
- indices_eu
- all_publishable_remaining (sp500_hors_top307 + indices_eu)

Réutilise la logique de fill-governance-heuristic-top307.py.
Output : src/data/v2-pipeline-enrich/<t>.json overrides_governance (merge field-by-field).
"""
...
def detect_country(t):
    ...
    if any(t.upper().endswith(s) for s in EU_SUF):
        return 'EU'
    if t.upper().endswith('.T') or t.upper().endswith('.HK') or t.upper().endswith('.KS') or t.upper().endswith('.AX'):
        return 'ASIA'
    return 'US'
...
def resolve_scope(scope):
    ...
    if scope == 'top307':
        return top307
    publishable = json.load(open('src/data/v1-9-publishable.json'))['tickers']
    sp500 = set(json.load(open('src/data/sp500-tickers.json')))
    top307_set = set(top307)
    pub_set = set(publishable)

    if scope == 'sp500_hors_top307':
        return sorted((pub_set - top307_set) & sp500)
    if scope == 'indices_eu':
        return sorted((pub_set - top307_set) - sp500)
    if scope == 'all_publishable_remaining':
        return sorted(pub_set - top307_set)
    raise ValueError(f'unknown scope: {scope}')

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--scope', default='top307',
                        choices=['top307','sp500_hors_top307','indices_eu','all_publishable_remaining'])
    parser.add_argument('--yf-cap', type=int, default=60)
    parser.add_argument('--stats-out', default='/tmp/governance-fill-stats-extended.json')
    args = parser.parse_args()

    tickers = resolve_scope(args.scope)
    print(f'Scope: {args.scope}, {len(tickers)} tickers')
    ...
    SAMPLE_KEYS = ['FRT','IBKR','AKZA.AS','SCA-B.ST','INGA.AS']
    ...
    for i, t in enumerate(tickers, 1):
        ...
        d['_governance_heuristic_source'] = f'fill-governance-heuristic-extended ({args.scope})'
    ...
    json.dump(stats, open(args.stats_out,'w'), indent=2, ensure_ascii=False)
    print(f"\n=== DONE ===")
