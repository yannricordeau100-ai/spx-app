export const meta = {
  name: 'kpi-history-pilot',
  description: 'Pilote extraction historique KPI complet 3 tickers',
  phases: [{ title: 'Extract' }],
}

const TICKERS = ['COF', 'BA', 'DD']
...
FICHIERS SOURCE:
- 10-Q: /Users/yann/Mettrik/docs/${t}/10-Q/*.htm.gz (30 filings)
- 10-K: /Users/yann/Mettrik/docs/${t}/10-K/*.htm.gz (10 filings)
...
3. Pour chaque filing (du plus ancien au plus recent), lire les 40000 premiers caracteres: zcat <fichier> 2>/dev/null | head -c 40000
   - Identifier le trimestre ("Three months ended March 31, 2022" -> Q1-2022, etc.). Fiscal decale = utiliser convention Q-FYxxxx.
...
FILINGS trop nombreux? Priorise couverture large: lire au moins 15 10-Q + tous les 10-K. Si limite de contexte, sample 1 sur 2 mais couvre 2016-2025.

Retour: appeler StructuredOutput avec ticker, ok (true si au moins 1 KPI a un history >= 8), max_periods (max des len(history) sur tous les KPIs finaux), notes (breve description problemes rencontres, ex: "3 KPIs non trouves", "format inhabituel HTML").`

phase('Extract')
const results = await parallel(TICKERS.map(t => () =>
  agent(prompt(t), { label: `extract:${t}`, schema: SCHEMA, effort: 'high' })
))
