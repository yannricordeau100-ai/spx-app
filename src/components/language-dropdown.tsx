 ...
 *
 * Remplace l'ancien LanguageSwitcher (binaire FR/US flags).
 *
      ...
      <AnimatePresence>
        {open && (
          <motion.ul
            ...
            role="listbox"
            className="absolute right-0 top-full z-50 mt-2 min-w-[200px] overflow-hidden rounded-lg border border-white/15 bg-[#0c0c10] shadow-2xl"
          >
            {LOCALES.map((loc) => {
                ...
                <li key={loc}>
                  <button
                    ...
                    className={`flex w-full items-center gap-2.5 px-3 py-2 text-left text-[13px] transition-colors ${
                      isActive ? "bg-violet-500/15 text-violet-100" : "text-zinc-300 hover:bg-white/[0.04]"
                    }`}
                  >
