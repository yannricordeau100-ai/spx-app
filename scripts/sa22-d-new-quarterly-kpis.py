...
LOG_PATH = ROOT / "sec-data/_meta/sa22-d-new-quarterly.log"
...
# Track keys with permanent failures (402 quota, 401 auth) pour les exclure
...
def extract_text(htm_gz: Path, max_chars: int = 12000) -> str:
    ...
    low = text.lower()
    # Recherche des sections les plus denses en KPIs sectoriels chiffrés.
    # Stratégie : trouver les zones financières clés (Note Segment Info, Revenue by *)
    # et concaténer plusieurs fenêtres pour maximiser la couverture.
    priority_anchors = [
        "segment information",
        ...
        "reportable segments",
        "table below presents details of our reportable",
    ]
    # Calcul taille par fenêtre pour rentrer dans max_chars total
    # Cible: 4 fenêtres maximum (3 anchors + MD&A), réparties équitablement
    win_size = max(2000, max_chars // 4)
    windows = []
    seen_pos = set()
    for kw in priority_anchors:
        idx = low.find(kw)
        if idx > 0 and not any(abs(idx - p) < 1500 for p in seen_pos):
            seen_pos.add(idx)
            start = max(0, idx - 200)
            windows.append(text[start : start + win_size])
            if len(windows) >= 3:
                break
    # Toujours inclure la section MD&A "Results of Operations" (apres boilerplate)
    md_idx = low.find("results of operations")
    if md_idx > 0:
        md2 = low.find("results of operations", md_idx + 100)
        target = md2 if md2 > 0 else md_idx
        windows.insert(0, text[target : target + win_size])
    if not windows:
        idx = low.find("management's discussion and analysis")
        start = idx + 3500 if idx > 0 else 0
...
def next_key() -> str:
    if not CEREBRAS_KEYS:
        return ""
    live = [k for k in CEREBRAS_KEYS if k not in DEAD_KEYS]
    if not live:
        return ""
    k = live[_key_idx[0] % len(live)]
    _key_idx[0] += 1
    return k


def call_llm(system: str, user: str, retries: int = 4) -> dict | None:
    for attempt in range(retries + 1):
        ...
        try:
            ...
            if r.status_code == 429:
                # backoff progressif sur TPM/RPM
                time.sleep(15 + attempt * 10)
                continue
            if r.status_code in (401, 402):
                # Key dead, retirer du pool
                DEAD_KEYS.add(key)
                continue
...
def process_ticker(ticker: str, fh=None) -> str:
    ...
    # Idempotence: skip si déjà extrait
    slug_check = slug_for_ticker(ticker)
    if (ENRICH_DIR / f"{slug_check}.sa22d.json").exists():
    ...
    # Take last 4 10-Qs avec 4 fenêtres ciblées chacun (10000 chars/file).
    # => 40k chars total ≈ 10k tokens prompt (OK avec TPM share)
    only_10q = [f for f in filings if "/10Q/" in str(f)]
    sel = only_10q[-4:] if only_10q else filings[-4:]
    docs_text = []
    for f in sel:
        txt = extract_text(f, max_chars=10000)
        if txt:
            docs_text.append(f"=== {f.name} ===\n{txt}")
    full = "\n\n".join(docs_text)[:42000]
    if len(full) < 3000:
        return "no-text"
    ...
    for k in new_kpis:
        ...
        entry = {
            ...
            "_fix_log": [f"SA22-D new quarterly KPI {short} via Cerebras gpt-oss-120b"],
        }
        valid.append(entry)
    ...
    # Écrit dans un fichier ISOLÉ <slug>.sa22d.json pour éviter races avec autres procs
    # qui modifient en parallèle <slug>.json (cron-cerebras-restart, enrich-risks, sa24g...).
    slug = slug_for_ticker(ticker)
    sa22d_path = ENRICH_DIR / f"{slug}.sa22d.json"
    payload = {
        "ticker": ticker,
        "_sa22_d_extracted_at": datetime.now().strftime("%Y-%m-%d %H:%M"),
        "model": "cerebras-gpt-oss-120b",
        "kpis": valid,
    }
    sa22d_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2))
...
def main():
    ...
    log(f"=== SA22-D START : {len(all_tickers)} tickers, {len(CEREBRAS_KEYS)} keys ===", fh)
