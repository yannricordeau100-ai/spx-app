#!/usr/bin/env python3
"""SA36 batch 08 — parse 10-Q HTM (inline XBRL) for US tickers, EU annual/semi for EU.

US tickers: convert period_type='year' KPIs to 'quarter' from 10-Q .htm.gz when
≥4 quarterly periods can be extracted.
EU tickers (DGE.L, EDP.LS, FORTUM.HE, III.L): need ≥4 half-year or quarterly periods;
if only annual-text/annual-report available, SKIP.
...
BATCH = '/tmp/sa36-batches/batch08.txt'
TODAY = '2026-06-03'
FIX_LOG_TAG = 'SA36-Claude'

EU_TICKERS = {'DGE.L', 'EDP.LS', 'FORTUM.HE', 'III.L'}
...
def process_ticker(ticker, apply_mode=False):
    ...
    if ticker in EU_TICKERS:
        ...
        if not sub_dirs_with_periods:
            return ['SKIP EU (only annual sources, no half-year/quarterly/interim/ad-hoc for ≥4 periods)']
...
def main():
    ...
    print(f'=== SA36 batch08 ({len(tickers)} tickers) apply={apply_mode} ===')
