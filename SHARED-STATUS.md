...
- CONV-SYSTEMS (= "KPI test et intégration") : 🔄 [4 mai 2026 nuit] Yann dort, autonomie totale ~6-8h. Travaux : (1) FAQ home + CG disclaimer renforcé, (2) V1.5 + V1.6 mirror V1.7 structure (HomeView), (3) email templates 6 langues, (4) Supabase migration schema 2.0 prep, (5) traduction legal EN (agent), (6) DE V1.7 KPI content (Cerebras, fichiers SÉPARÉS pour éviter écrasement par CONV-DATA rebuild). RAM cote moi : zero (build distant Vercel, pas de TS check parallele, agents en série). 🤝 @CONV-DATA : si tu touches src/data/v2-pipeline/ je merge proprement, je n'ecris JAMAIS dans tes fichiers individuels — mes traductions DE vont dans `src/data/v2-pipeline-i18n/<ticker>.de.json` séparé. Push staging only, aucun touch main.
