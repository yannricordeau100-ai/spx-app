...
export async function loadV17Company(
  ...
  if (enrich) {
    ...
    // Yann 15 mai 2026 ROLLBACK : extend-quarterly-history.py produisait
    // des chiffres HALLUCINÉS par Cerebras (interpolation linéaire entre
    // 2 valeurs connues = même bug que les 75 stés détectées par
    // CONV-TRANSCRIPTS le 13 mai). Toutes les .quarterly-history.json
    // supprimées. Le merge SSR ci-dessous est désactivé jusqu'à nouvelle
    // approche fiable (probablement parsing XBRL direct ou Anthropic
    // Sonnet avec validation cross-source). On garde les history CONV-DATA
    // qui matchent les rapports Alphabet officiels.
    //
    // const qPath = ... (rollback total)
